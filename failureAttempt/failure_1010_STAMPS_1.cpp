...
void compute(vector <int>& vStamps, data& dRes, int val) {
    for (int j = 0; j < vStamps.size(); j++) {
        for (int k = 1; k < vStamps.size(); k++) {
            for (int l = 1; l < vStamps.size(); l++) {
                for (int m = 0; m < vStamps.size(); m++) {
                    if (vStamps[m] == val) {
                        dRes = { val, 1, {vStamps[m]} };
                        return;
                    }
                    else if ( vStamps[m]+vStamps[l] == val && !( isInside(vStamps, vStamps[m]+vStamps[l]) ) ) {
                        if (m != l)
                            dRes = { val, 2, {vStamps[m], vStamps[l]} };
                        else
                            dRes = { val, 1, {vStamps[m], vStamps[l]} };
                        return;
                    }
                    else if ( vStamps[m]+vStamps[l]+vStamps[k] == val && !( isInside(vStamps, vStamps[m]+vStamps[l]+vStamps[k]) ) ) {
                        if (m != l && m != k && l != k)
                            dRes = { val, 3, {vStamps[m], vStamps[l], vStamps[k]} };
                        else if (m == l && m == k && l == k)
                            dRes = { val, 1, {vStamps[m], vStamps[l], vStamps[k]} };
                        else
                            dRes = { val, 2, {vStamps[m], vStamps[l], vStamps[k]} };
                        return;
                    }
                    else if ( vStamps[m]+vStamps[l]+vStamps[k]+vStamps[j] == val && !( isInside(vStamps, vStamps[m]+vStamps[l]+vStamps[k]+vStamps[j]) ) ) {
